...
/** TablelistPro adapter: fail-closed webhook verification + reservation normalisation. */
describe('TablelistAdapter', () => {
  function make(cfg: Record<string, string | undefined>) {
    const config: any = { get: (k: string) => cfg[k] };
    return new TablelistAdapter(config);
  }
  ...
  it('accepts a correctly signed body and rejects a tampered one', () => {
    const secret = 't4blesecret';
    const a = make({
      env: 'production',
      'connectors.tablelistWebhookSecret': secret,
    });
  ...
  it('normalises a Tablelist-shaped body into ReservationPayload (dollars → cents)', () => {
    const a = make({});
    ...
    expect(res.minSpendCents).toBe(25050);
    expect(Number.isInteger(res.minSpendCents)).toBe(true);
  });

  it('uses min_spend_cents as-is when the vendor sends integer cents', () => {
    const a = make({});
    const res = a.normalizeReservation({
      reservation_id: 'r10',
      party_size: 3,
    ...
    expect(res.minSpendCents).toBe(30000);
    expect(Number.isInteger(res.minSpendCents)).toBe(true);
  });
  ...
  it('returns deterministic stub reservations in stub mode', async () => {
    ...
    expect(rows[0].externalReservationId).toBe('tl_res_2001');
    expect(rows[0].status).toBe('booked');
    expect(rows[0].minSpendCents).toBe(50000);
  ...
  it('throws in live mode (api key configured)', async () => {
    const a = make({ 'connectors.tablelistApiKey': 'live-key' });
    await expect(a.fetchReservations('venue_1')).rejects.toThrow(
      'Tablelist live mode not configured in this build',
    );
  });
});
